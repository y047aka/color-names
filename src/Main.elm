...
import Css exposing (Color, backgroundColor, borderCollapse, collapse, color, hsl, hsla, left, padding, pct, px, textAlign)
import Css.Global exposing (descendants, each)
import Css.Reset.Customized exposing (customizedResetCss)
import HslaColor exposing (HslaColor, toColor, toCssString, toHsla)
...
colors : List { color : HslaColor, name : String }
colors =
    [ { color = white
      , name = "white"
      }
                ...
                [ fromUnstyled <|
                    let
                        colorCircle item =
                            ...
                            in
                            circle
                                [ cx <| 100 * (item.color |> toHsla |> .saturation) * cos rad
                                , cy <| 100 * (item.color |> toHsla |> .saturation) * sin rad
                                , r 5
                                , fill (item.color |> toColor |> Paint)
                                ]
                                []
                    in
                    svg
